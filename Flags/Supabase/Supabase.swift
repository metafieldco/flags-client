...
class Supabase: NSObject {
    ...
    func cleanup(uuid: String){
        DispatchQueue.global(qos: .background).async {
            do {
                ...
                try self.updateVideoRecord(uuid: uuid, updateReq: VideoTableUpdateRequest(state: .recyled)){ result in
                    switch result {
                    case .success:
    ...
    func markVideoActive(uuid: String){
        do {
            try self.updateVideoRecord(uuid: uuid, updateReq: VideoTableUpdateRequest(state: .active)){ result in
                switch result {
                case .success:
                    print("Successfully activated video with id: \(uuid)")
                case .failure(let err):
                    print("Error activating video: \(err) with id: \(uuid)")
                }
            }
        }catch{
            print("Runtime error activating video: \(error) with id: \(uuid)")
        }
    }
